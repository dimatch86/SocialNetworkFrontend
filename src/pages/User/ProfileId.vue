<template lang="pug">
  .profile.inner-page(v-if="getUsersInfo")
    .inner-page__main
      .profile__info
        profile-info(:info="getUsersInfo" :blocked="getUsersInfo.is_blocked_by_me" :friend="getUsersInfo.is_friend" :online="getUsersInfo.is_onlined")
      .profile__news
        .profile__tabs
          span.profile__tab.active {{ $t('posted') }} {{getUsersInfo.first_name}} ({{postedWall.length}})
        .profile__news-list
          news-block(v-for="news in postedWall" :key="news.id" :info="news")
  ...
  computed: {
    ...
    postedWall() {
      ...
      for (let key in this.getWall) {
        if (this.getWall[key].type === "POSTED") {
